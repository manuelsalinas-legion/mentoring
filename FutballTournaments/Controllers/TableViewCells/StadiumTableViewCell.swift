...
class StadiumTableViewCell: UITableViewCell {
    ...
    func loadInfo(_ teamDetails: TeamDetails?) {
        if let imageURL = URL(string: teamDetails?.strStadiumThumb ?? "") {
            self.imageStadium.loadRemote(url: imageURL)
        } else {
            self.imageStadium?.image = UIImage(named: "placeholderPicture")
        }
        
        if let strStadium = teamDetails?.strStadium {
            self.labelStrStadium.text = strStadium
        } else {
            self.labelStrStadium.text = "Uknown"
        }
    }
}
